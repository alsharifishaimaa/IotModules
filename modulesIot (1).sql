...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `body` = VALUES(`body`), `headers` = VALUES(`headers`), `queue_name` = VALUES(`queue_name`), `created_at` = VALUES(`created_at`), `available_at` = VALUES(`available_at`), `delivered_at` = VALUES(`delivered_at`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `user_id` = VALUES(`user_id`), `selector` = VALUES(`selector`), `hashed_token` = VALUES(`hashed_token`), `requested_at` = VALUES(`requested_at`), `expires_at` = VALUES(`expires_at`);
...
INSERT INTO `unit` (`id`, `nom_module`, `type_module`, `description`, `etat_module`, `donnees_mesurees`, `created_at`, `updateded_at`, `user_id`) VALUES
(1,	'Module Humidité2',	'Capteur d\'humidité',	'Capteur dédié à la mesure du taux d\'humidité dans l\'air',	'enPanne',	'Taux d\'humidité en pourcentage 20%',	'2024-05-20 23:16:51',	'2024-05-21 08:38:20',	0),
(2,	'Module de Surveillance de l\'Énergie',	'Analyseur de puissance',	'Appareil permettant de surveiller la consommation d\'énergie électrique en temps réel.',	'enPanne',	'Consommation d\'énergie en kilowatt-heure (kWh)',	'2024-05-20 23:19:11',	'2024-05-20 23:19:11',	0),
(3,	'Module GPS',	'Tracker GPS',	'Système de suivi par GPS pour localiser et suivre les véhicules ou objets',	'inactif',	'Coordonnées GPS (latitude, longitude)',	'2024-05-20 23:20:07',	'2024-05-20 23:20:07',	0),
(4,	'Module de Qualité de l\'Air',	'Capteur de qualité de l\'air',	'Module utilisé pour mesurer la qualité de l\'air en détectant divers polluants',	'enFonctionnement',	'Indice de qualité de l\'air (AQI)',	'2024-05-20 23:22:24',	'2024-05-20 23:22:24',	0),
(5,	'Module de Pression3',	'Capteur de pression',	'Capteur utilisé pour mesurer la pression atmosphérique',	'Actif',	'Pression en hectopascals (hPa)',	'2024-05-20 23:23:35',	'2024-05-21 11:35:56',	0)
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `nom_module` = VALUES(`nom_module`), `type_module` = VALUES(`type_module`), `description` = VALUES(`description`), `etat_module` = VALUES(`etat_module`), `donnees_mesurees` = VALUES(`donnees_mesurees`), `created_at` = VALUES(`created_at`), `updateded_at` = VALUES(`updateded_at`), `user_id` = VALUES(`user_id`);
...
ON DUPLICATE KEY UPDATE `id` = VALUES(`id`), `email` = VALUES(`email`), `roles` = VALUES(`roles`), `password` = VALUES(`password`), `is_verified` = VALUES(`is_verified`), `nom` = VALUES(`nom`), `prenom` = VALUES(`prenom`), `adresse` = VALUES(`adresse`), `ville` = VALUES(`ville`), `pays` = VALUES(`pays`);
